...
$bars = $wp_user_search->get_results();
?>
...
<div id="content" class="bars">
	...
	<div class="page-content">
		...
		<?php foreach ( $bars as $bar ) : ?>
			<div id="<?php echo $bar->ID; ?>" class="bar-info">
				<h3><?php echo get_user_meta( $bar->ID, 'nickname', true ); ?></h3>
				<div class="links">
					<?php if (strlen(trim($bar->user_url)) > 0) {
						echo '<a href="'.$bar->user_url.'" target="_blank">Site</a>';
					} ?>
					<?php if (strlen(trim(get_user_meta( $bar->ID, 'contact', true ))) > 0) {
						echo ' + <a href="mailto:'.get_user_meta( $bar->ID, 'contact', true ).'" target="_blank">Contact</a>';
					} ?>
					<?php if (strlen(trim(get_user_meta( $bar->ID, 'facebook', true ))) > 0) {
						echo ' + <a href="'.get_user_meta( $bar->ID, 'facebook', true ).'" target="_blank">Facebook</a>';
					} ?>
					<?php if (strlen(trim(get_user_meta( $bar->ID, 'twitter', true ))) > 0) {
						echo ' + <a href="http://twitter.com/'.get_user_meta( $bar->ID, 'twitter', true ).'" target="_blank">Twitter</a>';
					} ?>
				</div>
				<p>
					<?php echo get_user_meta( $bar->ID, 'about', true ); ?>
				</p>
				<div>
					<a href="#" class="show-all" data-id="<?php echo $bar->ID; ?>" data-action="show">SHOW ALL OF THIS MEMBER’S EVENTS</a>
				</div>
			</div>
		<?php endforeach; ?>
		</div>
		<div class="list">
			<span>All Bars</span>
			<?php foreach ( $bars as $bar ) : ?>
			    <a href="#" data-id="<?php echo $bar->ID; ?>"><?php echo get_user_meta( $bar->ID, 'nickname', true ); ?></a>
			<?php endforeach; ?>
		</div>
